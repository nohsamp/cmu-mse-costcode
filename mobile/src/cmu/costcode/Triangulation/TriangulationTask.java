...
public class TriangulationTask extends AsyncTask<Void, Void, Void> {
	...
	private boolean setPreference() {
		...
		if (!prefs.getBoolean("wifi_checkbox", true)) {
			Toast.makeText(context, "Make WiFi Triangulation enable",
					Toast.LENGTH_SHORT).show();
			return false;
		} else {
			bgrunFlag = prefs.getBoolean("bgrun_checkbox", true);
			initParams.put(Triangulation.TRIANG_METHOD,
					prefs.getString("triangulation_list", "WCL"));
			initParams.put(Triangulation.SCAN_NUMBER,
					Integer.valueOf(prefs.getString("wifi_scannum", "5")));
			initParams.put(Triangulation.NOISE_FILTER,
					prefs.getBoolean("noise_filter_checkbox", true));
			initParams.put("DUMMY", prefs.getBoolean("dummy", false)); // TODO:
																		// test
																		// purpose,
																		// remove
																		// it
	...
	protected Void doInBackground(Void... params) {
		...
		while (true) {
			...
			if (ap != null
					&& (category = triangulation.getNearestCategory(
							ap.getPosX(), ap.getPosY())) != null) {
				Intent intent = new Intent(
						ProximityIntentReceiver.PROXIMITY_ALERT);
