...
public class DatabaseAdaptor {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		private static final String SQL_CREATE_CUSTOMER = "CREATE TABLE "
				+ DbContract.CustomerEntry.TABLE_NAME + " ("
		...
		private static final String SQL_CREATE_LISTITEM = "CREATE TABLE "
				+ DbContract.ListItemEntry.TABLE_NAME + " ("
				+ DbContract.ListItemEntry.ROW_KEY
				+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
		...
		private static final String SQL_CREATE_AP = "CREATE TABLE IF NOT EXISTS "
				+ DbContract.AccessPointEntry.TABLE_NAME
				+ " ("
				+ DbContract.AccessPointEntry.AP_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
		...
		private static final String SQL_CREATE_VERSION = "CREATE TABLE IF NOT EXISTS "
				+ DbContract.InfoVersionEntry.TABLE_NAME
				+ " ("
				+ DbContract.InfoVersionEntry.INFOVERSION_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
				+ DbContract.InfoVersionEntry.INFO_NAME
				+ " TEXT, "
				+ DbContract.InfoVersionEntry.INFO_VERSION
				+ " TEXT, "
				+ DbContract.InfoVersionEntry.INFO_DESC + " TEXT" + " )";

		private static final String SQL_CREATE_ALERTS = "CREATE TABLE "
				+ DbContract.AlertsEntry.TABLE_NAME + " ("
				+ DbContract.AlertsEntry.ALERT_ID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
		...
		/**
		 * On upgrade of DB, discard all previous results, recreate original
		 * tables
		 */
		@Override
	...
	/**
	 * Return a new Customer object, created from DB data in 'customers' table
	 * with 'memberId'
	 * 
	 * @param memberId
	 * @return
	...
	public Customer dbGetCustomer(int memberId) {
		...
		Cursor mCursor = db.query(true, DbContract.CustomerEntry.TABLE_NAME,
				new String[] { DbContract.CustomerEntry.MEMBER_ID,
		...
		String firstName = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.CustomerEntry.NAME_FIRST));
		String lastName = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.CustomerEntry.NAME_LAST));
		String address = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.CustomerEntry.ADDRESS));
	...
	public ShoppingList dbGetItemList(int memberId) {
		...
		Cursor mCursor = db.query(true, DbContract.ListItemEntry.TABLE_NAME,
				new String[] { DbContract.ListItemEntry.ITEM_ID, },
				DbContract.ListItemEntry.MEMBER_ID + "=" + memberId, null,
				null, null, null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG, "Read list of ShoppingListItems for memberId "
					+ memberId + ". Found " + mCursor.getCount() + " matches.");
		} else {
			// No matches found
		...
		for (int i = 0; i < mCursor.getCount(); i++) {
			...
			long itemId = mCursor.getLong(mCursor
					.getColumnIndexOrThrow(DbContract.ListItemEntry.ITEM_ID));
		...
		Log.i(TAG, "Reading from DB: ShoppingList with "
				+ shoppingList.getItems().size() + " items");
		return shoppingList;
	}
	...
	public Map<String, ShoppingList> dbGetShoppingList(int memberId) {
		...
		// Create Customer's list of ListItems mapped to category
		Map<String, ShoppingList> shoppingList = new HashMap<String, ShoppingList>();
		...
		Log.i(TAG, "Reading from DB: ShoppingList with " + shoppingList.size()
				+ " categories");
		return shoppingList;
	}
	...
	public Item dbGetItemById(long itemId) {
		...
		Cursor mCursor = db.query(true, DbContract.ItemEntry.TABLE_NAME,
				new String[] { DbContract.ItemEntry.ITEM_ID,
				...
				DbContract.ItemEntry.ITEM_ID + "=" + itemId, null, null, null,
				null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG, "Read list of Items with itemId " + itemId + ". Found "
					+ mCursor.getCount() + " matches.");
		} else {
			// No matches found
		...
		String name = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_NAME));
		String description = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_DESCRIPTION));
		float price = mCursor.getFloat(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_PRICE));
		int quantity = mCursor.getInt(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_QUANTITY));
		int unit = mCursor.getInt(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_UNIT));
		String category = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_CATEGORY));
		String upc = mCursor.getString(mCursor
				.getColumnIndexOrThrow(DbContract.ItemEntry.ITEM_UPC));

		// Create Item object and return it
		Log.i(TAG, "Reading from DB: Item (id " + itemId + ") in category '"
				+ category + "': '" + description + "'");
		return new Item(itemId, category, name, quantity, price, unit,
				description, upc);
	}
	...
	public List<AccessPoint> dbGetAccessPoint() {
		...
		Cursor mCursor = db.query(true, DbContract.AccessPointEntry.TABLE_NAME,
				new String[] { DbContract.AccessPointEntry.BSSID,
						DbContract.AccessPointEntry.SSID,
						DbContract.AccessPointEntry.POSX,
						DbContract.AccessPointEntry.POSY, }, null, null, null,
				null, null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG, "Read list of AccessPoint. Found " + mCursor.getCount()
					+ " matches.");
		} else {
			// No matches found
		...
		for (int i = 0; i < mCursor.getCount(); i++) {
			...
			ap.setBssid(mCursor.getString(mCursor
					.getColumnIndexOrThrow(DbContract.AccessPointEntry.BSSID)));
			ap.setSsid(mCursor.getString(mCursor
					.getColumnIndexOrThrow(DbContract.AccessPointEntry.SSID)));
			ap.setPosX(mCursor.getFloat(mCursor
					.getColumnIndexOrThrow(DbContract.AccessPointEntry.POSX)));
			ap.setPosY(mCursor.getFloat(mCursor
					.getColumnIndexOrThrow(DbContract.AccessPointEntry.POSY)));
			apList.add(ap);
	...
	public List<Category> dbGetCategory() {
		...
		Cursor mCursor = db.query(true, DbContract.CategoryEntry.TABLE_NAME,
				new String[] { DbContract.CategoryEntry.CATEGORY_ID,
						DbContract.CategoryEntry.CAT_NAME,
						DbContract.CategoryEntry.POSX,
						DbContract.CategoryEntry.POSY,
						DbContract.CategoryEntry.DESC }, null, null, null,
				null, null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG, "Read list of Category. Found " + mCursor.getCount()
					+ " matches.");
		} else {
			// No matches found
		...
		for (int i = 0; i < mCursor.getCount(); i++) {
			...
			String name = mCursor.getString(mCursor
					.getColumnIndexOrThrow(DbContract.CategoryEntry.CAT_NAME));
			double x = mCursor.getFloat(mCursor
					.getColumnIndexOrThrow(DbContract.CategoryEntry.POSX));
			double y = mCursor.getFloat(mCursor
					.getColumnIndexOrThrow(DbContract.CategoryEntry.POSY));
		...
		Log.i(TAG, "Reading from DB: Category list with " + categoryList.size()
				+ " categories");
		return categoryList;
	}
	 ...
	 * @return new memberId
	 */
	public int dbCreateCustomer(String firstName, String lastName,
			String address) {
		...
		return (int) db.insert(DbContract.CustomerEntry.TABLE_NAME, null,
				values);
	}
	 ...
	 * @return
	 */
	public long dbCreateShoppingListItem(long newItemId, int memberId,
			boolean checked, int position) {
		// Create a new map of values, where column names are the keys
		ContentValues values = new ContentValues();
	 ...
	 * @return
	 */
	public long dbCreateItem(String name, String category, int quantity,
	...
	public void dbCreateCategory(Category category) {
		...
		values.put(DbContract.CategoryEntry.POSX, category.getLocation()
				.getLat()); // posx
		values.put(DbContract.CategoryEntry.POSY, category.getLocation()
				.getLon()); // posy
	...
	public int dbSetItemChecked(int memberId, int itemId, boolean checked) {
		...
		return db.update(DbContract.ListItemEntry.TABLE_NAME, values,
				selection, null);
	}
	 ...
	 * @return
	 */
	public int dbUpdateItemName(long itemId, String category, String name) { // ,
																				// int
																				// quantity,
																				// float
																				// price,
		...
		return db.update(DbContract.ItemEntry.TABLE_NAME, values, selection,
				null);
	}
	...
	public String dbGetVersion(String infoName) {
		...
		Cursor mCursor = db.query(DbContract.InfoVersionEntry.TABLE_NAME,
				new String[] { DbContract.InfoVersionEntry.INFO_VERSION, },
				DbContract.InfoVersionEntry.INFO_NAME + "= '" + infoName + "'",
				null, null, null, null, null);

		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG, "Read floorplan version. Found " + mCursor.getCount()
					+ " matches.");
			// Create new Customer object, return it
			return mCursor
					.getString(mCursor
							.getColumnIndexOrThrow(DbContract.InfoVersionEntry.INFO_VERSION));
		} else {
			// No matches found
	 ...
	 * @param version
	 *            The version of information
	 * @param description
	 *            Description about the information
	...
	public int dbSetVersion(String infoName, String version, String description) {
		...
		if (dbGetVersion(infoName) == null) {
			// insert
			return (int) db.insert(DbContract.InfoVersionEntry.TABLE_NAME,
					null, values);
		}
		// else update data
		else {
			// Which row to update, based on the infoName
			String selection = DbContract.InfoVersionEntry.INFO_NAME + "= '"
					+ infoName + "'";

			return db.update(DbContract.InfoVersionEntry.TABLE_NAME, values,
					selection, null);
		}
	}

	/**
	 * Return a map of all proximity alerts, tying category name to GPS
	 * coordinates
	 * 
	 * @param memberId
	 * @return
	...
	public Map<String, Location> dbGetProxAlerts() {
		...
		Cursor mCursor = db.query(true, DbContract.AlertsEntry.TABLE_NAME,
				new String[] { DbContract.AlertsEntry.CATEGORY_NAME,
						DbContract.AlertsEntry.LATITUDE,
						DbContract.AlertsEntry.LONGITUDE }, null, null, null,
				null, null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			Log.d(TAG,
					"Read list of Proximity Alerts, found "
							+ mCursor.getCount() + " matches.");
		} else {
			// No matches found
		...
		for (int i = 0; i < mCursor.getCount(); i++) {
			String category = mCursor
					.getString(mCursor
							.getColumnIndexOrThrow(DbContract.AlertsEntry.CATEGORY_NAME));
			Double latitude = mCursor.getDouble(mCursor
					.getColumnIndexOrThrow(DbContract.AlertsEntry.LATITUDE));
			Double longitude = mCursor.getDouble(mCursor
					.getColumnIndexOrThrow(DbContract.AlertsEntry.LONGITUDE));
	...
	/**
	 * Create a new proximity alert with name 'category' at location (latitude,
	 * longitude)
	 * 
	 * @param category
	 * @param latitude
	 ...
	 * @param category
	 */
	public int dbDeleteProxAlert(String category, double latitude,
			double longitude) {
		// Which row to delete, based on 'category'
		String selection = DbContract.AlertsEntry.CATEGORY_NAME + "='"
				+ category + "'" + " AND " + DbContract.AlertsEntry.LATITUDE
				+ "=" + latitude + " AND " + DbContract.AlertsEntry.LONGITUDE
				+ "=" + longitude;
	...
	public ShoppingListItem dbGetShoppingListItem(Item item) {
		...
		Cursor mCursor = db.query(true, DbContract.ListItemEntry.TABLE_NAME,
				new String[] { DbContract.ListItemEntry.CHECKED,
						DbContract.ListItemEntry.POSITION },
				DbContract.ListItemEntry.ITEM_ID + "=" + itemId, null, null,
				null, null, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			mCursor.moveToFirst();
			...
			// No matches found
			Log.d(TAG, "No ShoppingListItem with itemId (UPC) " + itemId
					+ " found.");
			return null;
		}

		boolean checked = mCursor.getInt(mCursor
				.getColumnIndexOrThrow(DbContract.ListItemEntry.CHECKED)) > 0;
		int position = mCursor.getInt(mCursor
				.getColumnIndexOrThrow(DbContract.ListItemEntry.POSITION));
