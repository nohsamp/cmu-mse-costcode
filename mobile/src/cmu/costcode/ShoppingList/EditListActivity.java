...
public class EditListActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Prepare AddItem view; Inflate XML layout for adding new item, add to
		// scrolling list
		addItemView = getLayoutInflater().inflate(
				R.layout.activity_edit_list_new, null);
		...
		LinearLayout itemList = generateEditListView(this,
	...
	private LinearLayout generateEditListView(Context ctx,
	...
	private View createEditRow(Context ctx, final Item item) {
		...
		View view = getLayoutInflater().inflate(
				R.layout.activity_edit_list_row, null);
		...
		deleteButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Log.i(TAG, "Deleting ListItem id " + item.getId()
						+ " from shopping list");
	...
	private void updateList() {
		LinearLayout shoppingItemList = (LinearLayout) ((ViewGroup) findViewById(R.id.editListScroll))
				.getChildAt(0);

		// Check if a new item is being added
		if (shoppingItemList.findViewById(R.id.editAddItem) != null) {
			ViewGroup prevItem = (ViewGroup) shoppingItemList
					.findViewById(R.id.editAddItem);
		...
		for (int i = 0; i < childCount; i++) {
			...
			if (listItem instanceof TextView) {
				...
				EditText itemTextView = (EditText) listItem
						.findViewById(R.id.editItemText);
				long itemId = (Long) listItem.getTag();
				// TODO: change this pronto; user shouldn't have to enter prices
				// or UPCs
				db.dbUpdateItemName(itemId, category, itemTextView.getText()
						.toString());// , 1, 0.0f, "111222333444");
			}
		}
	}
	...
	private void addNewItemInfo(View newItemView) {
		...
		EditText prevItemDescView = (EditText) newItemView
				.findViewById(R.id.editNewItemDescription);
		Spinner prevItemCatView = (Spinner) newItemView
				.findViewById(R.id.editNewItemCategories);
		EditText numpicQtyView = (EditText) newItemView
				.findViewById(R.id.editTextQuantity);
		EditText numpicPriceView = (EditText) newItemView
		...
		try {
			...
			newItemPrice = Float.parseFloat(numpicPriceView.getText()
					.toString());
		} catch (Exception e) {
		...
		long newItemId = db.dbCreateItem(newItemDesc, newItemCat, newItemQty,
				newItemPrice, newItemUpc);
		db.dbCreateShoppingListItem(newItemId, cust.getMemberId(), false, 0);
		Log.i(TAG, "Adding item=(" + newItemDesc + ", " + newItemCat + " ("
				+ newItemId + ") )");

		Item newItem = new Item(newItemId, newItemCat, newItemDesc, newItemQty,
				newItemPrice, 0, newItemDesc, newItemUpc);
	...
	/**
	 * Saves all items in list. If a ListItem is different than stored with
	 * Customer, update DB.
	 * 
	 * @param view
	 */
	...
	public void addItem(View view) {
		...
		ScrollView scroll = (ScrollView) parentView
				.findViewById(R.id.editListScroll);
		...
		Button voiceButton = (Button) addItemView
				.findViewById(R.id.editNewItemVoiceButton);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == VOICE_REQUEST_CODE && resultCode == RESULT_OK) {
			...
			ArrayList<String> voiceResult = data
					.getStringArrayListExtra(RecognizerIntent.EXTRA_RESULTS);

			// Set text contents of new item to most likely result
			TextView newItemText = (TextView) addItemView
					.findViewById(R.id.editNewItemDescription);
