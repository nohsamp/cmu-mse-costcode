...
public class WiFiScanner {
	...
	public WiFiScanner(WifiManager wm, Map<String, Object> initParams,
			Context context) {
		wifimanager = wm;
	...
	public void scanStart() {
		...
		List<AccessPoint> tempList = fp.getAccessPoints(); // get Floor plan
															// information
	...
	private void parseWIFIScanResult() {
		...
		for (int i = 0; i < mScanResult.size(); i++) {
			...
			if (i > 0 && oldResult.BSSID.compareTo(newResult.BSSID) != 0) {
				// if noiseFilterFlag is true and tCount is less than
				// scanNumber,
				// drop info from AP
