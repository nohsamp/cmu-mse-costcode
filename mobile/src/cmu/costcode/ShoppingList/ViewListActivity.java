...
public class ViewListActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		CustomerNFC customer = new CustomerNFC(ViewListActivity.this, this,
				db.dbGetItemList(cust.getMemberId()));
	}
	...
	private View createItemCheckbox(Context ctx, final Item inputItem) {
		...
		checkbox.setText(inputItem.getName() + " -  $" + inputItem.getPrice()
				+ ",  Qty:" + inputItem.getQuantity()); // + ",\tUPC: " +
														// inputItem.getUpc());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getItemId() == R.id.menu_web) {
			...
			CustomerWeb customerWeb = new CustomerWeb(this, cust.getMemberId(),
					db.dbGetItemList(cust.getMemberId()));
			...
			String remindMethod = prefs.getString("reminder_list",
					res.getStringArray(R.array.pref_reminder_list_values)[0]);

			if (item.getItemId() == R.id.menu_wifitriangulation) {
				// the second item in the array: GPS
				if (remindMethod.equals(res
						.getStringArray(R.array.pref_reminder_list_values)[1])) {
					...
					IntentFilter proximityFilter = new IntentFilter();
					proximityFilter
							.addAction(ProximityIntentReceiver.PROXIMITY_ALERT);
					pReceiver = new ProximityIntentReceiver();
					registerReceiver(pReceiver, proximityFilter);

					// Create Background Triangulation Task --> asynchronous
					// thread
					// create new since app has to reload the preference every
					// time
					try {
						tTask = new TriangulationTask(this);
					} catch (Exception e) {
	...
	public void sendList(View view) {
		CustomerWeb customerWeb = new CustomerWeb(view.getContext(),
				cust.getMemberId(), db.dbGetItemList(cust.getMemberId()));
		customerWeb.broadcastShoppingList(getCurrentFocus());
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK && data != null) {
			...
			try {
				...
				Toast.makeText(this, "Scan fail: not an item." + barcode,
	...
	/**
	 * Call an asynchronous GET request to the server to retrieve product
	 * information
	 * 
	 * @param requestUrl
	 * @param ctx
			...
			protected void onPostExecute(JSONObject jsonObjRecv) {
				if (jsonObjRecv == null) {
					...
					try {
						...
						Toast.makeText(ctx, "Added new item ",
								Toast.LENGTH_LONG).show();
					} catch (JSONException e) {
						Toast.makeText(ctx,
								"Invalid response; did not add item. :(",
								Toast.LENGTH_LONG).show();
						Log.e(TAG,
								"Error adding item from server:" + e.toString());
					}
				}
			}
		}.execute(requestUrl);
	...
	private void addItem(String category, String desc, int quantity,
		...
		Item newItem = new Item(newItemId, category, desc, quantity, price, 0,
				desc, upc);
