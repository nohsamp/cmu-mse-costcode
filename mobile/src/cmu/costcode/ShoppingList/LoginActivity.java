...
public class LoginActivity extends Activity {
	...
	private int validateUser(String email, String password) {
		/*
		 * TODO: Query DB for rows wither EMAIL=email and PWORD=password. - Take
		 * customerId from this row - getCustomer with that ID or - get the list
		 */
	...
	private void createDummyInfo() {
		if (dbHelper.dbGetCustomer(DUMMY_MEMBER_ID) == null) {
			int memberId = dbHelper.dbCreateCustomer("Kevin", "Crane",
					"515 S Aiken Ave");
			memberId = 1;
			Log.d(TAG, "Created Dummy member Kevin with memberId " + memberId
					+ ".");

			long itemId1 = dbHelper.dbCreateItem("Harry Potter, Book 3",
