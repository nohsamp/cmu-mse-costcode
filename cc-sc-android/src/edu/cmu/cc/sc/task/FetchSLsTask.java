...
public class FetchSLsTask extends AsyncTask<Void, Void, List<ShoppingList>> {
	...
	private int retrieveServerCategoryVersion() throws Throwable {
		...
		String strVersion = response.getPropertyAsString(ctx
				.getString(R.string.ws_category_property_version));
	...
	private void parseAndSaveCategoriesXML(SoapObject root) {
		...
		Object categoriesProperty = root.getProperty(ctx
				.getString(R.string.ws_category_property_categories));
		...
		if (categoriesProperty instanceof SoapObject) {
			...
			for (int i = 0; i < categoriesXml.getPropertyCount(); i++) {
				...
				if (categoryProperty instanceof SoapObject) {
					...
					category.setId((Long) categoryXml.getProperty(ctx
							.getString(R.string.ws_category_property_id)));
					category.setName(categoryXml.getProperty(
							ctx.getString(R.string.ws_category_property_name))
							.toString());
					category.setDescription(categoryXml.getProperty(
							ctx.getString(R.string.ws_category_property_desc))
							.toString());
	...
	private void loadLocalCategories() {
		...
		Map<Long, ItemCategory> categoriesMap = new HashMap<Long, ItemCategory>(
				categories.size());
	...
	private int retrieveServerMemberVersion() throws Throwable {
		...
		String strMemberVersion = response.getPropertyAsString(ctx
	...
	private void parseSLsXML(SoapObject root) {
		...
		for (int i = 0; i < root.getPropertyCount(); i++) {
			...
			if (slProperty instanceof SoapObject) {
				...
				sl.setId((Long) slXml.getProperty(ctx
						.getString(R.string.ws_sl_property_id)));
				sl.setName(slXml.getProperty(
						ctx.getString(R.string.ws_sl_property_name)).toString());
				sl.setDate(new Date((Long) slXml.getProperty(ctx
						.getString(R.string.ws_sl_property_date))));
				sl.setDescription(slXml.getProperty(
						ctx.getString(R.string.ws_sl_property_desc)).toString());
				sl.setVersion((Integer) slXml.getProperty(ctx
						.getString(R.string.ws_sl_property_version)));

				Object itemsXML = slXml.getProperty(ctx
						.getString(R.string.ws_sl_property_items));
	...
	private void parseSLItemsXML(SoapObject root, ShoppingList sl) {
		...
		for (int i = 0; i < root.getPropertyCount(); i++) {
			...
			if (slItemProperty instanceof SoapObject) {
				...
				// SL Item: ID
				// ---------------------------------------------------
				slItem.setId((Long) itemXml.getProperty(ctx
						.getString(R.string.ws_sl_item_property_id)));
				...
				// ---------------------------------------------------
				slItem.setCategory(getCategoryById(
						(Long) itemXml.getProperty(ctx
								.getString(R.string.ws_sl_item_property_category)))
						.getName());

				// ---------------------------------------------------
				// SL Item: Name
				// ---------------------------------------------------
				slItem.setName(itemXml.getProperty(
				...
				// SL Item: Quantity
				// ---------------------------------------------------
				slItem.setQuantity((Integer) itemXml.getProperty(ctx
						.getString(R.string.ws_sl_item_property_quantity)));

				// ---------------------------------------------------
				// SL Item: Price
				// ---------------------------------------------------

				int price = (Integer) itemXml.getProperty(ctx
						.getString(R.string.ws_sl_item_property_price));
				...
				// SL Item: Unit
				// ---------------------------------------------------
				slItem.setUnit((Integer) itemXml.getProperty(ctx
						.getString(R.string.ws_sl_item_property_unit)));

				// ---------------------------------------------------
				// SL Item: Description
				// ---------------------------------------------------
				slItem.setDescription(itemXml.getProperty(
	...
	private void saveSampleSLs() {
		...
		Item item = new Item(1, "Furniture", "Couch cushions", 1, 37, 1, null,
				"184930574960");
		items.add(item);
		item.setShoppingList(shoppingList);
		slItemDAO.save(item);
		item = new Item(2, "Food", "Pre-made sandwichs", 1, 5, 1, null,
				"185038672860");
		items.add(item);
		item.setShoppingList(shoppingList);
		slItemDAO.save(item);
		item = new Item(3, "Clothing", "Woolen jackets aplenty", 1, 68, 1,
				null, "123456789048");
		items.add(item);
		item.setShoppingList(shoppingList);
		slItemDAO.save(item);
